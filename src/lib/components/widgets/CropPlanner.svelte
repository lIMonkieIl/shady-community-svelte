...
<div class="flex items-center w-full h-full justify-center">
	<div class="flex gap-2 justify-center items-center w-full h-full overflow-scroll flex-wrap">
		...
		<label class="label w-48">
			<span>Grown</span>
			<input value={selectedSeed.inOrOut} readonly class="input" title="seed_cost" type="text" />
		</label>
		<label class="label w-48">
			<span>Grow Time</span>
			<input
		...
		<TabGroup class={'w-full px-5 py-3'} justify="justify-center">
			<Tab bind:group={tabSet} name="tab1" value={0}>
				<span>Yield</span>
			</Tab>
			<Tab bind:group={tabSet} name="tab2" value={1}>Sell</Tab>
			{#if showSetUpCost}
				<Tab bind:group={tabSet} name="tab3" value={2}>Set Up</Tab>
			{/if}

			<!-- Tab Panels --->
			<svelte:fragment slot="panel">
				{#if tabSet === 0}
					<div class="flex truncate flex-wrap gap-4 justify-center">
						...
						<table class="table table-hover max-w-sm table-compact">
							...
							<tbody>
								...
								<tr>
									...
									<td>
										<div
											class="input items-center p-1 justify-center flex rounded-container-token w-20"
										>
											$
											<input
												min="10"
